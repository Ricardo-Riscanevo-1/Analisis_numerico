...
m = (yrev[2]-yrev[1])/(xrev[2]-xrev[1])
pend[1] = m
coord[1] = 1
coord<-lineal(2,3,pend,coord,xrev,yrev)
print (coord)


cat(length(x)-1)
plot(datosX,datosY, pch=7, cex=0.5, col = "red", asp=1,xlab="X", ylab="Y", main="Cambiado el origen")
...
for(i in 1:(length(coord)-1))
{
  ...
  datosX1 = NULL
  datosY1 = NULL
  datosX1[1]= xrev[coord[i]]
  datosX1[2]= xrev[coord[i+1]]
  datosY1[1]= yrev[coord[i]]
  datosY1[2]= yrev[coord[i+1]]
  cat("X",datosX1,"\n")
  cat("Y",datosY1,"\n")
