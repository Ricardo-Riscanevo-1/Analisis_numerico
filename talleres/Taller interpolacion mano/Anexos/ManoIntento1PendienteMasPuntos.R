...
m = (y2[2]-y2[1])/(x2[2]-x2[1])
pend[1] = m
coord[1] = 1
coord<-lineal(2,3,pend,coord,x2,y2)
print (coord)


cat(length(x)-1)
plot(datosX,datosY, pch=7, cex=0.5, col = "red", asp=1,xlab="X", ylab="Y", main="Mano con más puntos")
...
for(i in 1:(length(coord)-1))
{
  ...
  datosX1 = NULL
  datosY1 = NULL
  datosX1[1]= x2[coord[i]]
  datosX1[2]= x2[coord[i+1]]
  datosY1[1]= y2[coord[i]]
  datosY1[2]= y2[coord[i+1]]
  cat("X",datosX1,"\n")
  cat("Y",datosY1,"\n")
  if(datosX1[1] - datosX1[2] != 0)
  {
    ...
    points(datosX1,datosY1, pch=7, cex=1, col = "gray", asp=1,xlab="X", ylab="Y", main="Mano con más puntos")
    par(new=TRUE)
