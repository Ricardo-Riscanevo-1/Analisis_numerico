...
plot(xrev,yrev,col="blue",main = "Cambiando el Origen")
length(xrev)
segmento_mano = NULL
datosx=NULL
datosy=NULL
...
for(i in 1:fin_ciclo)
{
  ...
  if(multiplicacion < 0) # CAMBIO DE SENTIO SI RESTA Y SU ANTERIOR TIENEN DIFERENTES SIGNOS 
  {
    ...
    datosx[1] = xrev[segmento_mano[1]]#TOMAR EL PRIMER DATO
    datosx[2] = xrev[segmento_mano[2]]#TOMAR EL PRIMER DATO
    datosx[3] = xrev[segmento_mano[punto_fin]]#TOMAR EL ULTIMO DATO
    
    datosy[1] = yrev[segmento_mano[1]]#TOMAR EL PRIMER DATO
    datosy[2] = yrev[segmento_mano[2]]#TOMAR EL PRIMER DATO
    datosy[3] = yrev[segmento_mano[punto_fin]]#TOMAR EL ULTIMO DATO
    
    cat("EL ultimo dato es: ",datosy[3])
    
    if(length(segmento_mano) > 3)
    {
      datosx[4] = xrev[segmento_mano[punto_fin-1]]
      datosy[4] = yrev[segmento_mano[punto_fin-1]]
