...
criterioEuclidiana <- function(xi,xf,yi,yf)
{
  resultado = sqrt((xf-xi)^2+(yf-yi)^2)
  return(resultado)
}

lineal <- function(d1,d2,pend,coord){
 
  if(d2 <=length(x))
  { 
    if( x[d2] == x[d1]) { m = 0.0000001 } 
    else {if( y[d2] == y[d1]) { m = 0.000001 } else{ m = (y[d2]-y[d1])/(x[d2]-x[d1])}}
    
    cat("resta",pend[length(pend)], "-",m,"=",abs(pend[length(pend)] - m),"\n")
    
    if( abs((pend[length(pend)] - m )) < 1)
    {
      cat("1) d1:",d1," d2:",d2,"\n")
      lineal(d1,d2+1,pend,coord)
    }
    else 
    {
      if( x[d2] == x[d2-1]) { m = 99999}
      else {if( y[d2] == y[d2-1]) { m = 0.000001 } else{ m = (y[d2]-y[d2-1])/(x[d2]-x[d2-1])}}
      #m = (y[d2]-y[d2-1])/(x[d2]-x[d2-1])
      pend[length(pend)+1] = m
      cat("2) d1:",d2-1," d2:",d2,"\n")
      cat("m:",m,"\n")
      coord[length(coord)+1] = d2-1
      lineal(d2,d2+1,pend,coord)
    }
  }
  else
  {
...
cuadratic <- function(d1,d2,pend,coorx)
{
  if(d2<=length(coord))
  {
    ...
    # if(m > 0)
    # {
    #   if( (m < pend[length(pend)]) && (m * pend[length(pend)] > 0))
    #   {
    #     pend[length(pend)+1] = m
    #     cuadratic(d2,d2+1,pend,coorx)
    #   }
...
pend[1] = m
coord[1] = 1
coord<-lineal(2,3,pend,coord)
print (coord)
m = (y[coord[2]]-y[coord[1]])/(x[coord[2]]-x[coord[1]])
#pend = NULL
#pend[1] = m
#coorx[1] = 1
#coord2 <- cuadratic(2,3,pend,coorx)
#coordx <-coord2[]
#print(coord2)
...
{
  print(i)
  datosX1 = x[coord[i]:coord[i+1]]
  datosY1 = y[coord[i]:coord[i+1]]
  cat("d1",x[coord[i]],",",y[coord[i]],"\n")
  cat("d2",x[coord[i+1]],",",y[coord[i+1]],"\n")
  polinomioLagrange = poly.calc(datosX1,datosY1)
  par(new=TRUE)
  #plot(datosX1,datosY1, pch=7, cex=0.5, col = "red", asp=1,xlab="X", ylab="Y", main="Diagrama", xlim=c(0,30), ylim=c(0,30))
  curve(polinomioLagrange,add=T,datosX1[1],datosX1[length(datosX1)])
}
# 
...
#pruebax = c(1,2,3,4,5)
#pruebay = c(2,4,8,16,32)
#polinomioLagrange = poly.calc(pruebax,pruebay)
#polinomioLagrange
#plot(pruebax,pruebay, pch=7, cex=0.5, col = "red", asp=1,xlab="X", ylab="Y", main="Diagrama", xlim=c(0,30), ylim=c(0,30))
#par(new=TRUE)
#curve(polinomioLagrange,add=T)